...
export default function Navbar({ onToggleMenu }) {
  ...
  return (
    <>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 hidden lg:block transition-all duration-300 ${
        scrolled
          ? 'bg-[#F5F2EC]/95 backdrop-blur-md border-b border-[#0a0a0a]/5'
          : 'bg-transparent'
      }`} style={{ fontFamily: "'Inter', sans-serif" }}>
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link href="/" className="text-xs font-extrabold tracking-[3px] uppercase transition-colors" style={{ color: scrolled ? '#0a0a0a' : 'white' }}>
            Bahía Shops
          </Link>
          ...
          <div className="flex items-center gap-6">
            <Link
              ...
              style={{ color: scrolled ? 'rgba(10,10,10,0.45)' : 'rgba(255,255,255,0.5)' }}
            >
              Explorar
            </Link>
            <button
              onClick={onToggleMenu}
              className="text-[13px] font-light transition-colors cursor-pointer"
              style={{ color: scrolled ? 'rgba(10,10,10,0.45)' : 'rgba(255,255,255,0.5)' }}
            >
              Menú
            </button>
            ...
            <div className={`flex items-center gap-3 transition-opacity duration-300 ${scrolled ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
              <Link href="/favoritos" className="text-[#0a0a0a]/40 hover:text-[#0a0a0a] transition" aria-label="Favoritos">
                <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                </svg>
              </Link>
              <Link href="/carrito" className="relative text-[#0a0a0a]/40 hover:text-[#0a0a0a] transition" aria-label="Carrito">
                <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <Link href="/perfil" className="text-[#0a0a0a]/40 hover:text-[#0a0a0a] transition" aria-label="Perfil">
                <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {/* ── BUSCADOR FLOTANTE ABAJO (desktop) ── */}
      ...
      {/* ── NAVBAR MOBILE ── */}
      <nav className="fixed top-0 left-0 right-0 z-50 lg:hidden bg-[#F5F2EC]/95 backdrop-blur-md h-14 border-b border-[#0a0a0a]/5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
        <div className="bg-[#F5F2EC]/95 backdrop-blur-md border-t border-[#0a0a0a]/5 px-6 py-2 flex justify-around items-center">
